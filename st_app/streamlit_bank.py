...
des caractéristiques.""")
        st.image("st_app/shema_multi_nomial.png", width=200)
...
plus proches dans l’espace des caractéristiques.""")
        st.image("st_app/shema_KNN.png", width=200)
...
de variables indépendantes.""")
        ...
        model_paths = {
            "Bernoulli Naive Bayes": "st_app/model_bern_pipe.joblib",
            "Multinomial Naive Bayes": "st_app/model_multi_pipe.joblib",
            "K-Nearest Neighbors": "st_app/model_knn_pipe.joblib",
            "Logistic Regression": "st_app/model_lr_pipe.joblib",
        }
...
"""
            ...
            principaux ainsi que de leurs regroupements thématiques. Cette visualisation est importante pour :

            - **Identifier les chevauchements** ou **Réévaluer le nombre de sujets** : Si les groupes sont trop nombreux et se chevauchent,
            il est possible de diminuer leur quantité. À l'inverse, si les catégories sont trop larges ou imprécises, on peut augmenter le nombre de sujets pour une analyse plus fine.
    ...
    if st.checkbox("#Cas pratique", key="cas_pratique"):
        ...
        stop_words_input = st.text_area(
            "Stop words ou mots a enlever de l'analyse (séparez les mots par un point virgule)", ""
        )
        user_stop_words = [
...
##############################################################################
    ...
    dans une démarche scientifique, il est toujours possible d’aller plus loin : chercher, expérimenter, améliorer.
