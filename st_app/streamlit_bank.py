...
"""
        ...
        if st.checkbox("Méthode Bertopics", key="bertopics1"):
            ...
            st.write("""Nous avons reussi a obtenir des catégories mais qui malheuresement par manque de temps ne nous apas permis de
...
##############################################################################
    ...
    Utilisation de modèles pré-entraînés : Les modèles disponibles sur des plateformes comme Hugging Face, tels que BERT ou RoBERTa, pourraient offrir une base solide pour améliorer encore les performances de nos analyses.

    Augmentation des données : Intégrer davantage de volumétrie, notamment sur les avis négatifs, permettrait de renforcer l’apprentissage et la généralisation du modèle.

    Segmentation des jeux de données : Entraîner le modèle en fonction de la longueur des mots ou d’autres caractéristiques spécifiques pourrait affiner les résultats.

    Expérimentation de nouveaux modèles : Tester des architectures plus avancées, comme les Transformers, pourrait offrir des performances inédites.

    Outre les aspects techniques, ce projet nous a également permis de grandir sur le plan humain. Travailler en groupe nous a appris la force du collectif, l'importance de la coordination et l’impact des perspectives multiples sur la résolution des problèmes. Nous avons constaté que, dans une démarche scientifique, il est toujours possible d’aller plus loin : chercher, expérimenter, améliorer.

    Enfin, ce travail nous ouvre à de nouvelles applications potentielles. Par exemple, nos résultats pourraient être adaptés à d’autres domaines, comme l’analyse des sentiments dans des enquêtes, ou le tri des avis prioritaires pour des entreprises cherchant à améliorer leur satisfaction client.

    En conclusion, ce projet est une belle illustration de la manière dont collaboration, curiosité et persévérance peuvent nous mener à des résultats significatifs, tout en nous invitant à continuer d’avancer.""")
