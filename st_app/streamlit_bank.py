...
de variables indépendantes."""
    ...
    if st.checkbox("Rééquilibrage des données"):
        st.write(
            ...
            Sous-échantillonnage (Undersampling) Cette méthode réduit la taille des classes majoritaires. Par exemple, la technique Tomek Links identifie les échantillons majoritaires se trouvant à proximité immédiate des exemples minoritaires et les élimine, atténuant ainsi le déséquilibre.
    ...
    if st.checkbox("Optimisation du scaler"):
        ...
        st.text(classification_report(y_test, y_pred))
        st.write(
            "Nous voyons que les résultats sont bons avec ces deux méthodes, car leur accurancy est équivalente 0.97 et 0.96. Maitenant tous dépend de ce que nous recherchons, concernant le recall (Faux Negatif ou positifs)."
...
"""
        ...
        if st.checkbox("Méthode Bertopics2", key="bertopics2"):   
                ...
                # Création du pipeline Camembert (ne pas mettre dans le cache car non hachable)
                def create_embedder():
                    return pipeline("feature-extraction", model="camembert-base", tokenizer="camembert-base")

                # Mise en cache pour le calcul des embeddings (uniquement les données traitables)
                @st.cache_data
                def generate_embeddings(avis_list):
                    # Création locale du pipeline
                    embedder = create_embedder()
                    embeddings = embedder(avis_list, truncation=True, padding=True, max_length=512)
                ...
                selected_company = st.selectbox(
                    ...
                    options=df_clean["company"].unique()  # Remplacez 'company' par la colonne appropriée
                )
                ...
                if len(df_filtered) > 0:
                    ...
                    with st.spinner(f"Encodage des avis pour {selected_company}..."):
                        avis_list = df_filtered["avis_clean"].tolist()  # Pas besoin de tuple ici
                        embeddings = generate_embeddings(avis_list)
